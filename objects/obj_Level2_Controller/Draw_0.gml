...
if (current_line < array_length(dialogue)) {
    ...
    // Flip portrait to the right side if it's Milly!
    if (data.s == "Milly") {
    ...
    draw_set_valign(fa_middle); // Centers the name perfectly inside its tag
    ...
    draw_set_valign(fa_top); // CRITICAL FIX: Stops the text from jumping when wrapping!
    ...
    // --- CLEANUP ---
    // Reset all alignments and fonts so it doesn't break other parts of your game
    draw_set_halign(fa_left);
