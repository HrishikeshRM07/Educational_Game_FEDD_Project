// --- DEBUG SKIP ---
if (keyboard_check_pressed(vk_escape) && !targeting_phase) room_goto(rm_Level5_PostBattle);
...
for (var i = 0; i < array_length(enemies); i++) {
    if (enemies[i][1] > 0) { 
        ...
        if (enemies[i][6] > 0) enemies[i][6] -= 0.05; 
    }
}
    ...
    if (!targeting_phase) {
		...
		if (active_char == 0) {
			...
			if (skill_map[menu_index] == 1) {
				...
				fairy_text = "Provides defense & damage debuff to all enemies (lasts 3 turns): \nFor this, we’re going to divide, but we may not get a whole number. For example if I have 15 apples and I\u0027m breaking it up into sets of twos,\nI\u0027ll have 7 sets of two apples with 1 apple remaining, which means that I\u0027ll have half a set. This means the answer to 15 " + chr(247) + " 2 is 7.5 sets of apples!";
			}
		} else {
			if (skill_map[menu_index] == 1) {
				fairy_text = "Doubles damage of Erin’s next attack: When something is squared, that means you’re multiplying it by itself one time. For example, if I have 3" + chr(178) + " that means that I have 3 x 3, which equals 9! You can think of the number in the exponent as the number of times something will multiply by itself. But be careful! You can’t add numbers when you do exponents, you can only multiply them.";
			} else if (skill_map[menu_index] == 2) {
...
if (all_dead && attack_timer <= 0 && battle_state == BattleState.PLAYER_MENU) {
    if (current_wave < max_waves) {
        if (win_timer == -1) { win_timer = 120; fairy_text = "Bria: Well done! Get ready for the next wave!"; }
        ...
        if (win_timer == 0) {
            ...
            if (current_wave == 2) {
                ...
                enemies = [ 
                    ["Parabolate", 70, room_width-252, 840, Parabolate, 0, 1.0, c_white, 0.0], 
                    ["GoldmanTall", 60, room_width-448, 714, GoldmanTall, 0, 1.0, c_white, 0.0],
                ...
                enemies = [ 
                    ...
                    ["Aundroid", 80, room_width-448, 840, Aundroid, 0, 1.0, c_white, 0.0],
                    ["Aundroid", 80, room_width-644, 840, Aundroid, 0, 1.0, c_white, 0.0] 
                ];
            } else if (current_wave == 5) {
                enemies = [ 
                    ["Absarf", 90, room_width-252, 840, Absarf, 0, 1.0, c_white, 0.0], 
                    ["Absarf", 90, room_width-448, 840, Absarf, 0, 1.0, c_white, 0.0],
        ...
        fairy_text = "Bria: You did it! All 5 waves are clear!";
        if (win_timer > 0) win_timer--;
        if (win_timer == 0) room_goto(rm_Level5_PostBattle); 
    }
}
...
if (player_hp <= 0 && milly_hp <= 0 && erin_hp <= 0) {
    ...
    if (lose_timer == 0) room_goto(rm_Level5Story);
}